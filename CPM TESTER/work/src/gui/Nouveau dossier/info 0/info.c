...
SElExceptionPtr EventFunctionCallback( const char* eventName, void* pData)
{
	...
	if(0==strcmp(eventName, "EVNT_GUI_INFO_SET"))
	{
		if(pData)
		{
		    ...
		    else if(0==strncmp(setting, "PANEL_MSG_VERSION_VALEO", strlen("PANEL_MSG_VERSION_VALEO")))
			{
				pdata = (char*)setting + strlen("PANEL_MSG_VERSION_VALEO:");
				EXCCHECKCVI( SetCtrlVal(gs_panel[PANEL], PANEL_MSG_VERSION_VALEO, pdata));
			}
				
			else if(0==strncmp(setting, "PANEL_MSG_VERSION_PSA", strlen("PANEL_MSG_VERSION_PSA")))
			{
				pdata = (char*)setting + strlen("PANEL_MSG_VERSION_PSA:");
				EXCCHECKCVI( SetCtrlVal(gs_panel[PANEL], PANEL_MSG_VERSION_PSA, pdata));
			}
			
			else if(0==strncmp(setting, "PANEL_MSG_TYPE_CAN", strlen("PANEL_MSG_TYPE_CAN")))
			{
				pdata = (char*)setting + strlen("PANEL_MSG_TYPE_CAN:");
				EXCCHECKCVI( SetCtrlVal(gs_panel[PANEL], PANEL_MSG_TYPE_CAN, pdata));
			}
