...
int CreatePie (int hpanel, pieInfo pieAttr, piePiece piece, int legend)
{
    ...
    SetCtrlAttribute (hpanel, pieID, ATTR_LABEL_LEFT, pieAttr.left + (pieAttr.width / 2) - (labelWidth / 2));
    SetCtrlAttribute (hpanel, pieID, ATTR_LABEL_COLOR, VAL_WHITE);
    ...
    for (i = 0; i < piece.numPieces; i++)
        ...
        if (piece.piecePercent[i] > 0)
            ...
            if (legend)
                ...
                SetCtrlAttribute (hpanel, pieID, ATTR_PEN_FILL_COLOR, VAL_TRANSPARENT);
