...
static SElExceptionPtr CallbackFncStartBarrier(const char* name, uint32_t state, void* pData);
...
static SElExceptionPtr RegisterSensorCallbacks( struct _STester* me)
{
	...
	EXCCHECK( me->SetSensorCallback(me, "INFRARED_BARRIER", CallbackFncStartBarrier, me));
			...
			is_station_60_ready)
	{
		...
		while( TRUE)
		{
			...
			if (step >= CHECK_TABLE_POITION && step < ROTATION_FINIHED_OK)
			{
				...
				me->SetSensor(me, "CLOSE_GRIPPER_STATION_60", FALSE);
				me->SetSensor(me, "CYLINDER_Z_DOWN_STATION_60", FALSE);
				...
				me->SetSensor(me, "OPEN_GRIPPER_STATION_60", TRUE);
				me->SetSensor(me, "CYLINDER_Z_UP_STATION_60",  TRUE);
...
static SElExceptionPtr CallbackFncButtonStart(const char* name, uint32_t state, void* pData)
{
	SElExceptionPtr	pexception = NULL;
	STesterPtr		me = (STesterPtr)pData;
	bool_t          status_micro_cam, status_barrier ,Mode_Manu;
	bool_t          sensor_plug_station_10, sensor_unplug_station_10;

	/* check where is the table */
	EXCCHECK( me->GetSensor(me, "MICRO_CAM", &status_micro_cam));
    EXCCHECK( me->GetSensor(me, "Auto_Manu", &Mode_Manu));
	...
	if(status_barrier == 1 && status_micro_cam == 1 && Mode_Manu==1)
	{
...
Error:
 ...
 /*---------------------------------------------- */
  #undef __FUNC__
#define __FUNC__	"CallbackFncStartBarrier"
static SElExceptionPtr CallbackFncStartBarrier(const char* name, uint32_t state, void* pData)
{
	SElExceptionPtr	pexception = NULL;
	STesterPtr		me = (STesterPtr)pData;
	...
	if(status_barrier == 1 && status_micro_cam == 1 && Mode_Manu == 0 )
		{
		EXCCHECK( me->eventAPI->PulseEvent(me->eventAPI, EVNT_JIG_CYCLE_START, NULL));
		}
...
static SElExceptionPtr CallbackFncButtonTotalStop(const char* name, uint32_t state, void* pData)
{
	...
	STesterPtr		me = (STesterPtr)pData;
	    ...
	    EXCCHECK( me->SetSensor(me, "VERRINE_RED", !state));
