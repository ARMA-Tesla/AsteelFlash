...
SElExceptionPtr Test_SMM(STracaPtr me,  char* Board_Ver_pFin ,bool_t *Subassembly)
{
	...
	//sprintf(chaine, "SELECT [SubAssembly] FROM [Board] WHERE [Board_Ver] like '%s' AND [SubAssembly] = 1 AND [Id_Famille]= 3 ",Board_Ver_pFin); 
	
	
	//Under this Board Ver pFin there is just one variant
	//sprintf(chaine, "SELECT [SubAssembly] FROM [Board] WHERE [Board_Ver] like '%s' AND [Id.Famille] = 3 ",Board_Ver_pFin);
