...
)
{
	...
	if ( psessions->AI_data.pData )
	{
		...
		memset( psessions->AI_channels, 0, sizeof(SChannelInfo)*16);
	}
 ...
 */
	...
	psessions->AI_data.pData = calloc( (size_t)(count * samples_per_channel),sizeof(float64));    
	psessions->AI_data.pChannelData = calloc( (size_t)samples_per_channel, sizeof(float64));
	psessions->AI_data.AllocatedSamples  = (ViInt32)samples_per_channel;
...
{	 
	...
	psearch = bsearch( &channel_info, psessions->AI_channels, 16, sizeof(SChannelInfo), CompareChannelInfo);
...
)
{
	...
	viCheckDAQ( DAQmxReadAnalogF64( psessions->ai, 
									(int32)psessions->AI_data.Samples,
									1.0e-3,
									DAQmx_Val_GroupByChannel,
									psessions->AI_data.pData,
									(int32)psessions->AI_data.AllocatedSamples*psessions->AI_data.AllocatedChannels,
									&psessions->AI_data.UsedForChannel, NULL));
